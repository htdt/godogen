#!/usr/bin/env python3
"""${AGENT_NAME} Stop hook for post-task visual verification."""
...
def run_verify(project_root: Path, metadata_only: bool) -> dict[str, object]:
    command = [
        "python3",
        str(project_root / "${HOOK_CONFIG_DIR}" / "hooks" / "verify_result.py"),
        "--project-root",
        str(project_root),
    ]
    if metadata_only:
...
def main() -> None:
    ...
    state_path = project_root / "${HOOK_CONFIG_DIR}" / "hooks" / "state" / f"{event.get('session_id', 'default')}.json"
    state = load_json(
        state_path,
        {
