...
Default to planning locally in the main thread. Only use a helper agent for planning if the planning work itself is broad enough that isolating it will keep the implementation context cleaner.
...
When something looks wrong in screenshots but the cause is not obvious, use the `visual-qa` skill in question mode to get a second pair of eyes. Run Gemini inline. If you need `--native` or `--both`, use a helper agent so the native reviewer starts from a fresh context.
...
3. **Inspect** the specific handoff with the `visual-qa` skill in dynamic or question mode — ask about the transition, not the static art.
