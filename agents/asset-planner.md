...
If `reference.png` exists (generated by the orchestrator), read it and derive the style from it — match its color palette, lighting mood, material feel, and visual identity. Describe what you see as a comma-separated style string.

If no reference image exists, invent a style that makes this specific game visually distinctive — mix media, reference art movements, blend unexpected influences. The generators handle any style well, so be bold.
