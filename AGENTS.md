...
- `publish.sh` — renders a runtime repo with `--engine {godot,bevy,babylon}`, `--agent {claude,codex,zcode,dsh}`
- `scripts/` — render helpers: `render_dir.py` (token substitution), `generate_codex_metadata.py` (Codex `openai.yaml`)

## Editing Rules

- Do not create or maintain `.claude/skills/`, `.agents/skills/`, `.zcode/skills/`, or `.dsh/skills/` in this source repo.
