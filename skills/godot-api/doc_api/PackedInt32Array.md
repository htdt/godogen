...
- append_array(array: PackedInt32Array) - Appends a PackedInt32Array at the end of this array.
...
- duplicate() -> PackedInt32Array - Creates a copy of the array, and returns it.
...
- get(index: int) -> int - Returns the 32-bit integer at the given `index` in the array. If `index` is out-of-bounds or negative, this method fails and returns `0`. This method is similar (but not identical) to the `[]` operator. Most notably, when this method fails, it doesn't pause project execution if run from the editor.
