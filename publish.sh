...
#
# Usage:
#   ./publish.sh --engine godot|bevy|babylon --agent claude|codex|zcode|dsh --out <dir> [--force]
#   ./publish.sh --engine godot|bevy|babylon --agent claude|codex|zcode|dsh <dir> [--force]
#
...
case "$AGENT" in
    ...
    zcode)
        MANIFEST="AGENTS.md"
        SKILLS_DIR_REL=".zcode/skills"
        AGENT_NAME="ZCode"
        ASSET_SKILL_COMMAND="/asset-gen"
        ;;
    dsh)
        MANIFEST="AGENTS.md"
        SKILLS_DIR_REL=".dsh/skills"
        AGENT_NAME="dsh"
        ASSET_SKILL_COMMAND="asset-gen"
        ;;
    *) echo "error: --agent must be claude, codex, zcode, or dsh" >&2; usage; exit 1 ;;
esac
