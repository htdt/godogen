...
#
# Usage:
#   ./publish.sh --engine godot|bevy|babylon --agent claude|codex|prime --out <dir> [--force]
#   ./publish.sh --engine godot|bevy|babylon --agent claude|codex|prime <dir> [--force]
#
...
case "$AGENT" in
    ...
    prime)
        MANIFEST="AGENTS.md"
        SKILLS_DIR_REL=".prime/agent/skills"
        AGENT_NAME="Prime Agent"
        ASSET_SKILL_COMMAND="/skill:asset-gen"
        ;;
    *) echo "error: --agent must be claude, codex, or prime" >&2; usage; exit 1 ;;
esac
