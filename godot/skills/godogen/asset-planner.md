...
For Godot projects, keep runtime-loaded outputs under `assets/`. When a task needs to load a generated file at runtime, record both the repo path and the Godot resource path (`res://assets/...`).
...
For runtime loading, Godot uses `res://` paths. `assets/img/car.png` becomes `res://assets/img/car.png`.
...
```markdown
- **Assets:**
  - `car` GLB model (`assets/glb/car.glb`, Godot path `res://assets/glb/car.glb`) — scale to 4m long
  - `grass` texture (`assets/img/grass.png`, Godot path `res://assets/img/grass.png`) — tile every 2m via UV scale
```
